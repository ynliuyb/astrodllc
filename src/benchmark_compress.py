...
def extract_blocks(arr, block_size):
    ...
    return (arr.reshape(h//nrows, nrows, -1, ncols)
               .swapaxes(1,2)
               .reshape(-1, nrows, ncols)), h, w

# method to reconstruct image from the extracted pixel-blocks从提取的像素块重建图像
...
def cifar(exp):
    ...
    return cifar.data[np.random.choice(len(cifar.data), size=(100, 100), replace=False)[exp]]
