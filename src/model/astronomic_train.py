...
class Model(nn.Module):
    def __init__(self, xs=(3, 576, 576), nz=1, zdim=4, nprocessing=1, kernel_size=3, resdepth=2,
                 reswidth=256, dropout_p=0., tag='', root_process=True):
        super().__init__()
        # default: disable compressing mode 默认值：禁用压缩模式
        ...
        # create loggers 创建日志记录器
        self.tag = tag
        if root_process:
            current_time = datetime.now().strftime('%b%d_%H-%M-%S')
            log_dir = os.path.join(
                'runs/astronomic/', current_time + '_' + socket.gethostname() + tag)
        ...
        # distribute ResNet blocks over latent layers 在潜在层上分布ResNet块
        resdepth = [0] * (self.nz)
        i = 0
        ...
        self.gen_mu = nn.Sequential(
            ...
            modules.WnConv2d(self.reswidth,
                             ...
                             padding,
                             init_scale=0.1),
            # shape: [4,16,16] -> [1,32,32]
            modules.UnSqueeze2d(factor=2)
        )

        # the scale parameter of the bottom (zi = 1) generative model is modelled unconditional 底部（zi=1）生成模型的尺度参数是无条件建模的
    ...
    def infer(self, i):
        ...
        def distribution(given):
            ...
            if i == 0:
                ...
                # 如果压缩，输入会被压平，所以我们必须把它转换回张量
                if self.compressing:
                    h = h.view((-1,) + self.xs)  # (3072,)-->(10, 1, 2160, 2560)
                # also, when NOT compressing, the input is not scaled from [0,255] to [-1,1]
                else:
                    h = (h - (ac.astro_tail/2)) / (ac.astro_tail/2)

                # (10, 1, 2160, 2560)-->(10, 16, 1080, 1280)
                h = self.infer_in(h)

                # (10, 16, 1080, 1280)-->(10, 16, 1080, 1280)
                h = self.infer_res0(h)

                # (10, 16, 1080, 1280)-->(10, 16, 1080, 1280)
                h = self.infer_res1(h)

                # mu parameter of the conditional Logistic distribution(10, 25, 1080, 1280)
                mu = self.infer_mu(h)

                # scale parameter of the conditional Logistic distribution 条件Logistic分布的尺度参数
                # clamp the output of the scale parameter between [0.1, 1.0] for stability 将尺度参数的输出夹在[0.1, 1.0]之间以保持稳定
                scale = 0.1 + 0.9 * self.sigmoid(self.infer_std(h) + 2.)   #####???????

            # deeper latent layers
            else:
                # if compressing, the input is flattened, so we'll have to convert it back to a Tensor
                # 如果压缩，则输入被展平，因此我们必须将其转换回张量
                if self.compressing:
    ...
    def generate(self, i):
        ...
        def distribution(given):
            ...
            # bottom latent layer
            if i == 0:
                # input convolution(10,25,1080,1280)-->(10,16,1080,1280)
                h = self.gen_in(h)

                # processing ResNet blocks-->(10,16,1080,1280)
                h = self.gen_res1(h)

                # other ResNet blocks-->(10,16,1080,1280)
                h = self.gen_res0(h)

                # mu parameter of the conditional Logistic distribution (10,1,2160,2560)
                mu = self.gen_mu(h)

                # scale parameter of the conditional Logistic distribution 条件Logistic分布的尺度参数
                # set a minimal value for the scale parameter of the bottom generative model
                scale = ((2. / ac.astro_tail) / ac.astro_bits) + modules.softplus(self.gen_std)
    ...
    # 将数据作为输入并输出ELBO的所有分量+潜在样本的函数
    def loss(self, x):
        # tensor to store inference model losses 存储推理模型损失
        ...
        for i in range(self.nz):
            ...
            # sample untransformed sample from Logistic distribution (mu=0, scale=1) 样本未转换样本来自逻辑斯特分布
            ...
            # generative model
            # get the parameters of inference distribution i given z //mu(10,1,2160,2560) scale(1,2560,2160)
            ...
            if i == 0:
                ...
                logp = torch.sum(random.discretized_logistic_logp(mu, scale, x, ac.astro_bits), dim=1)  # ******#
                logrecon = logp
        ...
        logp = torch.sum(random.logistic_logp(torch.zeros(1, device=x.device), torch.ones(1, device=x.device), z),
                         dim=2)
    ...
    def reconstruct(self, x_orig, device, epoch):
        ...
        # scale up from [-1.1] to [0,255]
        x_cont = (x_cont * (ac.astro_tail/2)) + (ac.astro_tail/2)

        # esnure that [0,255]
        x_sample = torch.clamp(x_cont, 0, ac.astro_tail) #?

        # scale from [0,255] to [0,1] and convert to right shape
        x_sample = x_sample.float() / ac.astro_tail
        x_orig = x_orig.float() / ac.astro_tail
...
def warmup(model, device, data_loader, warmup_batches, root_process):
    ...
    # prepare initialization batch 准备初始化批处理
    for batch_idx, (image, _) in enumerate(data_loader):                      ##############################
        # stack image with to current stack 到当前堆栈的堆栈图像
        warmup_images = torch.cat((warmup_images, image), dim=0) \
            if batch_idx != 0 else image

        # stop stacking batches if reaching limit 如果达到极限，则停止堆叠批次
    ...
    # do one 'special' forward pass to initialize parameters 执行一个“特殊”正向传递来初始化参数
...
def train(model, device, epoch, data_loader, optimizer, ema, log_interval, root_process, schedule=True, decay=0.99995):
    ...
    for batch_idx, (batch, _) in enumerate(data_loader):
        ...
        # copy the mini-batch in the pre-allocated data-variable 复制预分配数据变量中的小批量
        data.copy_(batch)

        # evaluate the data under the model and calculate ELBO components评估模型下的数据并计算ELBO分量
        ...
        if root_process and batch_idx % log_interval == 0 and log_interval < nbatches:
            ...
            print(
                f'Train Epoch: {epoch} [{batch_idx}/{nbatches} ({100. * batch_idx / len(data_loader):.0f}%)]\tLoss: {elbo.item():.6f}\tGnorm: {total_norm:.2f}\tSteps/sec: {(time.time() - start_time) / (batch_idx + 1):.3f}')
...
def test(model, device, epoch, ema, data_loader, tag, root_process):
    ...
    if root_process:
        ...
        if elbo < model.best_elbo and not np.isnan(elbo):
            ...
            if not os.path.exists(f'params/astronomic/'):
                os.makedirs(f'params/astronomic/')
            torch.save(model.state_dict(), f'params/astronomic/{tag}')
            # if epoch % 5 == 0:
            #     torch.save(model.state_dict(), f'params/astronomic/epoch{epoch}_{tag}')
            print("saved params\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', default=3, type=int, help="seed for experiment reproducibility")
    parser.add_argument('--nz', default=ac.astro_nz, type=int, help="number of latent variables, greater or equal to 1")
    ...
    parser.add_argument('--interval', default=500, type=int, help="interval for logging/printing of relevant values")
    parser.add_argument('--epochs', default=ac.astro_epochs, type=int, help="number of sweeps over the dataset (epochs)")
    parser.add_argument('--blocks', default=ac.astro_blocks, type=int, help="number of ResNet blocks")
    parser.add_argument('--width', default=ac.width, type=int, help="number of channels in the convolutions in the ResNet blocks")
    ...
    parser.add_argument('--batch', default=ac.astro_batch, type=int, help="size of the mini-batch for gradient descent")
    parser.add_argument('--dist', default=0, type=int,help="distribute (1) over different gpu's and use Horovod to do so, or not (0)")
    parser.add_argument('--lr', default=4e-4, type=float, help="learning rate gradient descent")
    ...
    # define the "root process": only one of the gpu's has to log relevant values定义“根进程”：只有一个gpu必须记录相关值
    # set only one gpu as root process仅将一个gpu设置为根进程
    root_process = True
    if distributed and not hvd.rank() == 0:
        root_process = False

    # set GPU/CPU options
    if ac.astro_use_cuda and torch.cuda.is_available():
        use_cuda = True
        cudastring = "cuda" if distributed else f"cuda:{gpu}"
        device = torch.device(cudastring)
    else:
        use_cuda = False
        device = "cpu"
    ...
    kwargs = {'num_workers': 1, 'pin_memory': True} if distributed else {}

    # set data pre-processing transforms
    # transform_ops = ac.transform_ops
    # transform_ops = transforms.Compose([transforms.ToTensor(), transforms.Grayscale()])

    # store data shape 存储数据形状
    xs = ac.xs
    zdim = ac.zdim
    ...
    # data_path = ac.astro_path
    # print("PATH:" + os.path.abspath(ac.astro_path))
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_set, num_replicas=1, rank=0)
    test_sampler = torch.utils.data.distributed.DistributedSampler(test_set, num_replicas=1, rank=0)
    ...
    # setup exponential moving average decay (EMA) for the parameters.设置参数的指数移动平均衰减(EMA)
    ...
    # initial test loss
    test(model, device, 25, ema, test_loader, tag, root_process)
    ...
    l_name=str(ac.astro_nz)
    np.save("testloss1080*1280_nz=" + l_name +".npy", testloss)  # 保存为.npy格式